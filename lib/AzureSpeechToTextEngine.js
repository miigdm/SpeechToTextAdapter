...
class AzureSpeechToTextEngine {
  ...
  constructor(config = {}, sendTranscription) {
    ...
    const pushStream = sdk.AudioInputStream.createPushStream(sdk.AudioStreamFormat.getWaveFormatPCM(24000, 16, 1));
  ...
  pushData(data) {
    if (data && data.length) {
      logger.debug('entro');
      // const data16 = alawmulaw.mulaw.decode(data);
      // const buffer16 = Buffer.from(data16.buffer);
      // logger.debug(buffer16);
      // logger.debug(this.stream, 'aca');
      // this.stream.write(buffer16.buffer);
      this.stream.write(data);
      // logger.debug(this.stream, 'aqui');
    }
  }
