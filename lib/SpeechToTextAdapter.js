...
const SpeechToTextEngine = require('./AzureSpeechToTextEngine');
const url = require('url');
const Config = require('../config');

const DEFAULT_PORT = Config.variables.port;
...
function setActiveSpeechEngine(speechToTextEngine, webSocket) {
  ...
  speechToTextEngine.on('error', (error) => {
    logger.error(error.message, 'SpeechToTextEngine encountered an error: ');
    const errorMessage = {
      error: error.message,
    };
  ...
  function sendTranscription(transcription) {
    const transcriptResponse = {
      results: [
        {
          final: true,
          alternatives: [{ transcript: transcription, confidence: 1 }],
        },
      ],
      results_index: 0,
    };
    logger.debug('Response sent:', transcriptResponse);
    webSocket.send(JSON.stringify(transcriptResponse));
  }
    ...
    if (typeof data === 'string') {
      try {
        ...
        if (message.action === 'start') {
          ...
          const config = Config.variables;
          logger.debug(config, 'config for engine: ');

          // Create a speech to text engine instance, must implement the
          // NodeJS Stream API
          speechToTextEngine = new SpeechToTextEngine(config, sendTranscription);
          logger.debug('engine is listening');
          const listeningMsg = {
            state: 'listening',
          };
      ...
      if (speechToTextEngine) {
        ...
        speechToTextEngine.pushData(data);
      }
    } else {
